...
int main()
{
    ...
    // -------------------------
    Shader treeShader("resources/shaders/tree_model.vs", "resources/shaders/tree_model.fs");
    Shader skyboxShader("resources/shaders/skybox.vs", "resources/shaders/skybox.fs");
    Shader nShader("resources/shaders/n_mapp.vs", "resources/shaders/n_mapp.fs");
    Shader pShader("resources/shaders/p_mapp.vs", "resources/shaders/p_mapp.fs");
    Shader blinnShader("resources/shaders/blinn.vs", "resources/shaders/blinn.fs");
    Shader blendShader("resources/shaders/blend.vs", "resources/shaders/blend.fs");
    // load models
    // -----------
    Model ourModel(FileSystem::getPath("resources/objects/tree/trees.obj"));
    ...
    // load textures
    // -------------
    unsigned int treeTexture = loadTexture(FileSystem::getPath("resources/objects/tree/branch1Color.png").c_str());
    ...
    blendShader.use();
    blendShader.setInt("blend", 0);

    blinnShader.use();
    blinnShader.setInt("blinn", 0);

    // load textures
    // -------------
    unsigned int diffuseMap = loadTexture(FileSystem::getPath("resources/objects/tree/bark1Color.png").c_str());
    unsigned int normalMap  = loadTexture(FileSystem::getPath("resources/objects/tree/bark1Normal.png").c_str());
    unsigned int depthMap = loadTexture(FileSystem::getPath("resources/objects/tree/bark1Roughness.png").c_str());
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // face culling(makes skybox dissappear or appear)
        ...
        modelTree = glm::translate(modelTree, glm::vec3(0.0f, 0.0f, 0.0f)); // translate it down so it's at the center of the scene
        modelTree = glm::scale(modelTree, glm::vec3(2.0f, 2.0f, 2.0f));	// it's a bit too big for our scene, so scale it down
        skyboxShader.setMat4("modelTree", modelTree);
        ourModel.Draw(treeShader);

        float time = glfwGetTime();

        pointLight.position = glm::vec3(8.0f * cos(time), 8.0f, 8*sin(time));
        ...
        treeShader.setFloat("material.shininess", 64.0f);
        ...
        nShader.use();
        nShader.setMat4("mProjection", mProjection);
        nShader.setMat4("mView", mView);

        blinnShader.use();
        glm::mat4 blinnProjection = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
        glm::mat4 blinnView = camera.GetViewMatrix();
        blinnShader.setMat4("blinnProjection", blinnProjection);
        blinnShader.setMat4("blinnView", blinnView);
        // set light uniforms
        ...
        nModel = glm::rotate(model, glm::radians((float)glfwGetTime() * -10.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0))); // rotate the quad to show normal mapping from multiple directions
        nShader.setMat4("nModel", nModel);
        ...
        pModel = glm::rotate(pModel, glm::radians((float)glfwGetTime() * -10.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0))); // rotate the quad to show parallax mapping from multiple directions
        pShader.setMat4("pModel", model);
        pShader.setVec3("viewPos", camera.Position);
        pShader.setFloat("heightScale", heightScale); // adjust with Q and E keys
